...
```
  const tuple: [number, string, object] = [1, "xiang", {}];
...
type nameAlias = "xiang" | "bobo"

function getNames(n: nameAlias): string{
  return n
}

console.log(getNames('xiang'))   // "xiang"  如果输入其他则会报错
...
const p2:Person = { name: "xiang", age: 28 };
...
```
getValue<string>('xiang'); // 定义 T 为 string 类型
...
let val = getValueByKey({ name: "xiang", age: 18, gender: "male" }, "name");
console.log(val); // xiang
