...
+ `ref` 更适合基本类型，也可以包裹对象
+ `reactive` 更适合对象、数组、`Map`、`Set` 等结构化数据

在实际项目里，两者都很常用。不是说 `reactive` 不重要，而是你可以先掌握 `ref`，再补充理解 `reactive` 的适用场景。
...
这里的问题不在于“解构语法不能用”，而在于你修改的是解构出来的局部变量，不会再触发原始 `ref` 的响应式更新。
...
使用 `props` 时，要注意“直接解构后是否还保留响应性”。

在早期 Vue3 版本中，直接解构 `props` 通常会丢失响应性，因此更稳妥的写法是使用 `props.foo`，或者先用 `toRefs(props)` / `toRef(props, 'foo')`。
```js
<script setup>
const props = defineProps({
  foo: String
...
如果你使用的是 Vue 3.5+，`<script setup>` 对 `props` 解构已经有更好的支持，但面试回答时最好不要把它说成绝对的“永远不能解构”，而应该结合版本来说明。
