...
console.log(window.name);  //xiang
lookName();                //xiang
window.lookName();         //xiang
```
...
```js
(function(){
    window.location.href= 'https://xiang.gitee.io/learing_record';
    console.log(123);
}())
```
上述代码不会立即执行页面跳转，而是会先输出123，然后才会跳转到xiang页面
